        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Acesse todas as análises realizadas e filtre por sentimento.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="h-10 pl-9 pr-4 rounded-lg border border-white/10 bg-card text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary transition-all w-52"
            ...
            <>
                    ...
                    <Fragment key={item.id}>
                      <TableRow
                        className={cn(
                          ...
                          expandedRows.has(item.id) && "bg-white/5",
                        )}
                        onClick={() => toggleRow(item.id)}
                      >
                        {/* Coluna Protocolo — substitui o ID */}
                        <TableCell>
                          {item.numero_protocolo ? (
                        ...
                        <TableCell>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-8 w-8 text-muted-foreground group-hover:text-white"
                          >
                                ...
                                <div className="p-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
                                  <div className="lg:col-span-1 space-y-3 border-r border-white/5 pr-6">
                                    ...
                                    {[
                                      ...
                                      {
                                        label: "Objetividade",
                                        value: item.objectivity,
                                      },
                                      {
                                        label: "Resolutividade",
                                        value: item.resolutiveness,
                                      },
                                    ].map((m) => (
                                      <div
                                        key={m.label}
              ...
              <div className="p-4 border-t border-white/5 flex items-center justify-between bg-black/20">
                <p className="text-sm text-muted-foreground">
                  Página{" "}
                  <span className="font-medium text-white">{data.page}</span> de{" "}
